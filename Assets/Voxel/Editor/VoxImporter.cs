...
public class VoxImporter: ScriptedImporter
{
    ...
    {
        // name of asset
        string name = Toolbox.NameFromPath(ctx.assetPath);
        for (int i = 0; i < sizes.Count; i++)
        {
            ...
            if(!usePivot) pivot = Vector3.zero;
            GameObject voxModel = _models[i];
            byte[,,] grid = voxModel.GetComponent<VoxModel>().grid;

            VoxelData so = ScriptableObject.CreateInstance<VoxelData>();
            so.map = new VoxelMap();
            so.map.size = size;
            //so.grid = grid;
            ...
            so.palette.SetTexture();

            for (int j = 0; j < 256; j++)
            {
                VoxPalette.MaterialType mat = VoxelMaterialImport.GetMaterialType(_materialsProps, (byte)j);
                so.palette.matRefs[j] = mat;
            }
            

            so.name = string.Concat(name, " Data");
            so.palette.name = string.Concat(name, " Palette");
            ...
            for (byte x = 0; x < size.x; x++)
            {
                for (byte y = 0; y < size.y; y++)
                {
                    ...
                    {
                        byte c = grid[x, y, z];
                        var matType = so.palette.matRefs[c];
                        byte matIndex = VoxelMaterialImport.GetMaterialindex(matType);
                        // get material index from palette
                        
                        Vector2 colorUv = Voxel.GetColorUv(c);
                        Vector2[] voxeluvs = colorUv.ToArray(4);

                        if (subMeshTriangles[matIndex] == null) subMeshTriangles[matIndex] = new List<int>();
                        if (c > 0)
                        {
                            var vox = new Voxel(x, y, z, c, matIndex);
                            so.map.v.Add(vox);
                            int3 direction = Voxel.zero;
                            ...
                            {
                                // we need bottom face
                                direction = Voxel.down;
                                d = vertices.Count;
                                
                                vertices.AddRange(VoxelMesh.GetFaceVertex(vox.Position, direction, pivot).ToVector3());
                                subMeshTriangles[vox.MeshIndex].AddRange(VoxelMesh.GetSubMeshTriangles(d));
                                normals.AddRange(direction.ToArray(4).ToVector3());
                            ...
                            {
                                // we need back face
                                direction = Voxel.back;
                                d = vertices.Count;
                                vertices.AddRange(VoxelMesh.GetFaceVertex(vox.Position, direction, pivot).ToVector3());
                                subMeshTriangles[matIndex].AddRange(VoxelMesh.GetSubMeshTriangles(d));
                                normals.AddRange(direction.ToArray(4).ToVector3());
                                uv.AddRange(voxeluvs);
                            }
                        }
            ...
            so.map.v = so.map.v.OrderBy(v => v.Position.y).ThenBy(v => v.Position.x).ThenBy(v => v.Position.z).ToList();
            ...
            {
                ctx.AddObjectToAsset(string.Concat(Toolbox.NameFromPath(ctx.assetPath), " datas"), so);
                ctx.AddObjectToAsset(string.Concat(Toolbox.NameFromPath(ctx.assetPath), " palette"), so.palette);
            }
...
public class VoxImporterEditor : ScriptedImporterEditor
{
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.PropertyField(serializedObject.FindProperty("usePivot"), new GUIContent("Use Pivot ?"));
...
public static class VoxelMaterialImport{
    internal static byte GetMaterialindex(VoxPalette.MaterialType matType){
        switch(matType)
        {
            case VoxPalette.MaterialType.DIFFUSE:
                return 0;
            case VoxPalette.MaterialType.METAL:
                return 1;
            case VoxPalette.MaterialType.GLASS:
                return 2;
            case VoxPalette.MaterialType.EMIT:
                return 3;
            default:
                return 0;
        }
    }
    internal static byte GetMaterialindex(VoxDict[] _materialsProps, byte c){
        return GetMaterialindex(GetMaterialType(_materialsProps, c));
    }

    internal static VoxPalette.MaterialType GetMaterialType(VoxDict[] _materialsProps, byte j){
        string matType = _materialsProps[j] == null ? "_diffuse" : _materialsProps[j].GetValue("_type");
        switch (matType)
        {
            case "_diffuse":
                return VoxPalette.MaterialType.DIFFUSE;
            case "_metal":
                return VoxPalette.MaterialType.METAL;
            case "_glass":
                return VoxPalette.MaterialType.GLASS;
            case "_emit":
                return VoxPalette.MaterialType.EMIT;
            default:
                return VoxPalette.MaterialType.DIFFUSE;
        }
    }
